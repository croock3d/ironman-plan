...
async function renderEisPdfPage(idx, pageNum) {
  ...
  try {
    ...
    // Dopasuj do szerokości kontenera (bez poziomego scrollu)
    // i renderuj w fizycznych pikselach (ostro na retina)
    const containerWidth = (canvasWrap.clientWidth || 340);
    const naturalViewport = page.getViewport({ scale: 1 });
    const cssScale = containerWidth / naturalViewport.width;
    const renderScale = cssScale * dpr;

    const cssW = containerWidth;
    const cssH = Math.round(naturalViewport.height * cssScale);

    const canvas = document.createElement('canvas');
    canvas.className = 'eis-pdf-canvas';
    // CSS size = dopasowana do szerokości kontenera
